...
def load_data(filename='/home/nannau/notebooks/nannau/data/cfis_ps_segue_gaia.fits', dust_filename='/home/nannau/notebooks/nannau/data/polecount_dust.fits', monte=True):
    ...
    df['u'] = df['u'].values - 4.239*EBV # I'm pretending these are SDSS band for u, but it is not and PS for the others!
    df['g'] = df['g'].values - 3.172*EBV #
    df['r'] = df['r'].values - 2.271*EBV #
    df['i'] = df['i'].values - 1.682*EBV #
    df['z'] = df['z'].values - 1.322*EBV #
    df['y'] = df['y'].values - 1.087*EBV #
    df['G'] = df['G'].values - 0.85926*EBV # Assume extinction coefficiants from Malhan+ 2018b
    df['BP'] = df['BP'].values - 1.06794*EBV
    df['RP'] = df['RP'].values - 0.65199*EBV
    
    return df

def load_data_glob(filename='/home/nannau/notebooks/nannau/data/cfis_ps_segue_gaia.fits', dust_filename='/home/nannau/notebooks/nannau/data/polecount_dust.fits'):
